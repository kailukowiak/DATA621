...
Transformation of variables is less needed for logistic regresion because normallacy is not a requriment. However we will transform some variables, which have a large skewness, to see if they aid in prediction. 
...
This is interesting because we can see how different the coefficents are even though it has the same variables.
...
logX <- testDF %>% dplyr::select(-target)
logX <- data.matrix(X)
